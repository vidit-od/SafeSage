        ...
        <div className="md:w-3/5 py-20 md:py-9  pr-10">
                    ...
                    <div className="flex justify-start my-5 w-full">
                        <div className=" bg-gray-100 px-5 flex rounded-full items-center w-2/3">
                            ...
                            <input type="text" name="" id="" className=" bg-transparent outline-none mx-2 w-full" placeholder="Search title or keyword here" value={filter} onChange={(e)=>setfilter(e.target.value)}/>
                        </div>
                        <ExploreButton filter={filter}/>
                    </div>
                </div>
    )
}

const ExploreButton:React.FC<{filter:string}> = ({filter})=>{
    console.log(filter);
    const navigate = useNavigate();
    const Search = ()=>{
        navigate(`/stories-filter?filter=${filter}&tag=`)
    }
    return (
        <button className="px-5 py-3 mx-2 bg-blue-custom1 text-white rounded-full transition-all duration-500 flex justify-center items-center group relative" onClick={Search}>
